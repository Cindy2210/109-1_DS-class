...
node_t *deleteNode(void *target, node_t **root, int (*cp)(void *A, void *B), void(*print_node)(void*))///刪除BST
{
    ...
    if( cp((void*)target, (void*)(*root)) == -1)///target value > root value
        (*root)->left = deleteNode(target, &((*root)->left), cp, print_node);

    else if(cp((void*)target, (void*)(*root)) == 1)///target value > root value
        (*root)->right = deleteNode(target, &((*root)->right), cp, print_node);
...
void treeEqual(node_t *tree1, node_t *tree2, int(*cp)(void *A, void *B), int *flag, void(*print_node)(void*))///比較兩個BST是否相同
{
    ...
    treeEqual(tree1->left, tree2->left, cp, flag, print_node);
    treeEqual(tree1->right, tree2->right, cp, flag, print_node);
